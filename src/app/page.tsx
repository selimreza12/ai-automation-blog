  ...
  return (
    <div className="space-y-16">
      ...
      <section className="text-center py-12 space-y-6">
        <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold bg-brand-500/10 text-brand-600 dark:text-brand-500 border border-brand-500/20">
          <Zap className="w-3.5 h-3.5" /> Built by Md. Selim Reza • AI Solutions Architect
        </span>
        ...
        <div className="flex flex-wrap justify-center gap-3 pt-2">
          <Link
            href="/tools"
            className="bg-brand-500 hover:bg-brand-600 text-dark-900 font-bold px-6 py-3 rounded-xl text-sm transition shadow-lg shadow-brand-500/20 flex items-center gap-2"
          >
            Hire for Custom Automation <ArrowRight className="w-4 h-4" />
          </Link>
          <Link
            href="/blog"
            className="bg-slate-200 dark:bg-dark-800 hover:bg-slate-300 dark:hover:bg-dark-700 text-slate-900 dark:text-white font-semibold px-6 py-3 rounded-xl text-sm transition"
          >
      ...
      <section className="p-8 rounded-2xl bg-white dark:bg-dark-800/80 border border-slate-200 dark:border-dark-700 space-y-6 shadow-sm">
        <div className="flex items-center gap-2 text-brand-600 dark:text-brand-500 text-xs font-bold uppercase tracking-wider">
          <ShieldCheck className="w-4 h-4" /> Verified Proof-of-Work & Portfolio
        </div>
        <h2 className="text-2xl font-extrabold text-slate-900 dark:text-white">Production Utilities Built by Selim</h2>
      ...
      {featured && (
        <section>
          <div className="p-8 rounded-2xl bg-white dark:bg-dark-800 border border-slate-200 dark:border-dark-700 hover:border-brand-500/50 transition shadow-sm">
            ...
            <Link href={`/blog/${featured.slug}`}>
              <h2 className="text-2xl sm:text-3xl font-bold mt-2 text-slate-900 dark:text-white hover:text-brand-500 transition">
                {featured.title}
              </h2>
            </Link>
            <p className="mt-3 text-slate-600 dark:text-slate-300 leading-relaxed text-sm">{featured.excerpt}</p>
            <div className="mt-6 flex items-center justify-between">
              <span className="text-xs text-slate-500 dark:text-slate-400 flex items-center gap-1">
                <Clock className="w-3.5 h-3.5" /> {featured.readingTime}
              </span>
              <Link href={`/blog/${featured.slug}`} className="text-brand-600 dark:text-brand-500 text-sm font-semibold flex items-center gap-1">
      ...
      {recent.length > 0 && (
        <section>
          <h2 className="text-2xl font-extrabold text-slate-900 dark:text-white mb-6">Latest Automation Workflows</h2>
          <div className="grid md:grid-cols-2 gap-6">
            {recent.map((post) => (
              <article key={post.slug} className="p-6 rounded-xl bg-white dark:bg-dark-800/60 border border-slate-200 dark:border-dark-700 flex flex-col justify-between shadow-sm">
                <div>
                  <span className="text-xs font-semibold text-brand-600 dark:text-brand-500 flex items-center gap-1 mb-2">
                    <Tag className="w-3 h-3" /> {post.category}
                  </span>
                  <Link href={`/blog/${post.slug}`}>
                    <h3 className="text-lg font-bold text-slate-900 dark:text-white hover:text-brand-500 transition">{post.title}</h3>
                  </Link>
